...
class DiglibModelSeries extends JModel
{
	public function getAseries()
	{
		$series_id = JRequest::getInt('id');

		$row = JTable::getInstance('series', 'DiglibTable');
		$row->load($series_id);
	...
	public function getSeries()
	{
		...
		$query->select('s.id as series_ids,s.name as series');
		$query->from('#__series as s');
		$query->order('s.name');
	...
	public function getAlldetails()
	{
		...
		$query->where("bs.series='{$series_id}'");
		$query->group('b.id');
		$query->order('b.id');
